...
/*
    @wordpress-plugin
    Plugin Name: Better Messages - WebSocket Version
    ...
    @fs_premium_only /inc/premium.php,/addons/calls.php,/inc/mini.php,/assets/js/bp-messages-premium.js,/assets/js/bp-messages-premium.js.LICENSE.txt,/assets/sounds/calling.mp3,/assets/sounds/calling.ogg,/assets/sounds/dialing.mp3,/assets/sounds/dialing.ogg
*/
        ...
        public function enqueue_js( $include_dynamic_data = true )
        {
            ...
            if ( bpbm_fs()->is__premium_only() ) {
                ...
                if ( $this->realtime ) {
                    ...
                    $turn_server = apply_filters( 'bp_better_messages_turn_server', 'turn.bpbettermessages.com' );
                    $script_variables['socket_server'] = $socket_server;
                    $script_variables['turn_server'] = $turn_server;
                    $script_variables['site_id'] = BP_Better_Messages_Premium()->site_id;
                    if ( is_user_logged_in() ) {
                        $script_variables['secret_key'] = sha1( $script_variables['site_id'] . BP_Better_Messages_Premium()->secret_key . get_current_user_id() );
                    }
                    $script_variables['EPN'] = $this->settings['enablePushNotifications'];
                    $script_variables['callRequestTimeLimit'] = $this->settings['callRequestTimeLimit'];
                    $script_variables['offlineCallsNotifications'] = $this->settings['offlineCallsNotifications'];
                    ...
                    if ( $this->settings['enablePushNotifications'] === '1' ) {
                        $vapid_keys = BP_Better_Messages()->premium->get_vapid_keys();
                        $script_variables['vapidPublicKey'] = $vapid_keys['publicKey'];
                        $script_variables['PNworker'] = site_url( '/' );
                        if ( $include_dynamic_data ) {
                            $script_variables['savePushNotificationsNonce'] = wp_create_nonce( 'save_user_push_subscription' );
                        }
                    }
                    ...
                    if ( $this->settings['encryptionEnabled'] === '1' ) {
                        ...
                        if ( is_user_logged_in() ) {
                            $script_variables['encrypt_key'] = $this->premium->get_user_secret_key( get_current_user_id() );
                            $script_variables['site_encrypt_key'] = $this->premium->get_site_secret_key();
                        }
                        
                        #$dependencies[] = 'bpbm-aes-256-js';
                    }
                    
                    $current_user = get_userdata( get_current_user_id() );
                    $script_variables['me'] = [
                        'name'   => base64_encode( ( !empty($current_user->display_name) ? $current_user->display_name : $current_user->user_login ) ),
                        'link'   => base64_encode( bp_core_get_userlink( $current_user->ID, false, true ) ),
                        'avatar' => base64_encode( BP_Better_Messages_Functions()->get_avatar( $current_user->ID, 40 ) ),
                    ];
                    ...
                    if ( !empty(BP_Better_Messages()->settings['badWordsList']) ) {
                        ...
                        $script_variables['badWordList'] = base64_encode( json_encode( $words_list ) );
                    }
            ...
            if ( bpbm_fs()->is__premium_only() ) {
                $file_name = 'bp-messages-premium.js';
            }

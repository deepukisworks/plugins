...
class EH_Stripe_Payment extends WC_Payment_Gateway {
    ...
    public function create_stripe_customer($token, $order, $user_email = false) {
        ...
        try{
           ...
           //check customer token is exist for the logged in user
            $customer = get_user_meta($logged_in_userid, '_stripe_customer_id', true);
             ...
             //create stripe customer 
            if (empty($customer)) { 
                $customer = $this->create_stripe_customer( $payment_method, null, $current_user_obj->user_email );

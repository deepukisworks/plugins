...
class GMW_PS_PT_Form_Settings {
	...
	public function default_settings( $settings, $form ) {
		...
		if ( 'posts_locator' === $form['component'] ) { 
			...
			$settings['search_results']['orderby']     = 'distance:Nearest Distance,post_title:Name,post_modified:Last Updated,post_date:Ending Soonest';
		}
			...
			if ( 'ajax_forms' !== $form['addon'] ) {
				...
				$form_fields['page_load_results']['orderby'] = array(
					...
					'options'    => array(
						...
						'publish_date'     => __( 'Publish date', 'gmw-premium-settings' ),
					),
					'attributes' => array(),
					'priority'   => 95,
				);
				...
				$form_fields['search_results']['orderby'] = array(
					...
					'type'        => 'text',
					'default'     => '',
					'placeholder' => 'ex. distance:Nearest Distance,post_title:Name,post_modified:Last Updated,post_date:Ending Soonest',
					'label'       => __( 'Orderby', 'gmw-premium-settings' ),

...
function um_post_registration_approved_hook( $user_id, $args ) {
	um_fetch_user( $user_id );
		//update_user_meta($user_id,'mycred_default','1');
		//echo $newcoins_points.'newcoins_points';
	UM()->user()->approve();
...
function um_post_registration_checkmail_hook( $user_id, $args ) {
	um_fetch_user( $user_id );
//update_user_meta($user_id,'mycred_default','10');
...
function um_post_registration_pending_hook( $user_id, $args ) {
	um_fetch_user( $user_id );
//update_user_meta($user_id,'mycred_default','100');
	UM()->user()->pending();
}
...
function um_after_insert_user( $user_id, $args ) {
	 ...
	 * ?>
	 */
	// update_user_meta($user_id,'mycred_default','1010');
...
function um_send_registration_notification( $user_id, $args ) {
	...
	if ( ! empty( $emails ) ) {
		foreach ( $emails as $email ) {
			...
			/* 	$existing_check_referral=get_user_meta($user_id,'referral_link',true);
				$referral_name=explode("mref=",$existing_check_referral);
				$final_name=$referral_name[1];
				$the_user = get_user_by('login', $final_name);
				$the_user_id = $the_user->ID;
				$coins_points=get_user_meta($the_user_id,'mycred_default',true);
				$update_coins=$coins_points+1000;
		$newcoins_points=update_user_meta($the_user_id,'mycred_default',$update_coins); */
		//$newcoins_points=update_user_meta($user_id,'mycred_default','10000000');
			} else {
...
function um_check_user_status( $user_id, $args ) {
	...
	global $wpdb;
	$existing_check_referral=get_user_meta($user_id,'referral_link',true);
//	echo $existing_check_referral.'existing_check_referral';
	$referral_name=explode("mref=",$existing_check_referral);
	$final_name=$referral_name[1];
	//echo $final_name.'final_name';
	$the_user = get_user_by('login', $final_name);
//	echo '<pre>'; print_r($the_user);
    $the_user_id = $the_user->ID;
	//echo $the_user_id.'the_user_id';
	//echo $the_user_id.'the_user_id';
	$coins_points=get_user_meta($the_user_id,'mycred_default',true);
	//echo $coins_points.'coins_points'.'<br>';
	$update_coins=$coins_points+1000;
	update_user_meta($the_user_id,'mycred_default',$update_coins);
	   //$query = $wpdb->query("UPDATE ".$wpdb->prefix."usermeta SET meta_value = '1500' WHERE user_id = '".$user_id."' and meta_key='mycred_default'");
...
//update_user_meta($user_id,'mycred_default','1005');
	//return $user_id;
}

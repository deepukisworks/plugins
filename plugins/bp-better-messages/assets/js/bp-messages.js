...
(function ($) {
    ...
    function initialInit() {
        isRtl = $('html[dir="rtl"]').length !== 0;
        bpMessagesWrap          = $(".bp-messages-wrap:not(.bp-better-messages-list, .bm-threads-list, #bp-better-messages-mini-mobile-open)");
        ...
        } else if ($(".bp-messages-wrap:not(.bp-better-messages-mini,.bp-better-messages-list,#bp-better-messages-mini-mobile-container,.bm-threads-list) .threads-list").length > 0) {
            __threads = true;
        }
            ...
            extensions: {
                "mention": new BMTCMention({
                    ...
                    renderPanelContent: function (panelEl, currentMentionText, selectMentionCallback, extensionObject) {
                        ...
                        var usersJson = chatContainer.find('[data-users-json]').attr('data-users-json');
                        ...
                        var users = jQuery.parseJSON(atob(usersJson));
        ...
        if ($('.bp-messages-wrap #send-to:not(.ready)').length > 0) {
            ...
            if(BP_Messages['disableUsersSearch'] === '0') {
                ...
                $('.bpbm-users-avatars-list .bpbm-users-avatars-list-item').click(function(event){
                    ...
                    var el = $(this);
                    var name  = el.find('.bpbm-users-avatars-list-item-name').text();
                    var image = el.find('.bpbm-users-avatars-list-item-avatar img').parent().html();
                    var nicename = el.attr('data-nicename');
                    sentTo.add(nicename);
                    $(container).find('.taggle_sizer').text('');
                    $('#send-to li.taggle:last .taggle_text').html('<span class="bpbm-avatar">' + image + '</span><span class="bpbm-name">' + name + '</span>');
                });
                ...
                $(input).autocomplete({
                    ...
                    select: function (event, data) {
                        ...
                        if (event.which === 1) {
                            ...
                            var itemhtml = '';
                            if( typeof data.item.img !== 'undefined' ) {
                                itemhtml += '<span class="bpbm-avatar">' + data.item.img + '</span>';
                            }

                            itemhtml += '<span class="bpbm-name">' + data.item.label + '</span>';

                            $('#send-to li.taggle:last .taggle_text').html(itemhtml);
                        }
                    },
                    ...
                    var itemhtml = '';
                    if( typeof item.img !== 'undefined' ) {
                        itemhtml += '<span class="bpbm-avatar">' + item.img + '</span>';
                    }

                    itemhtml += '<span class="bpbm-name">' + item.label + '</span>';
    ...
    function openMobileFullScreen( url ) {
        ...
        var usedHeight = 0;
        var chatHeaderHeight = bpMessagesWrap.find('.chat-header').outerHeight();
        usedHeight           = usedHeight + chatHeaderHeight;
